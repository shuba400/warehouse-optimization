...
const int num_destinations = 10;    // num of destinations where robot can leave items after picking
...
vector<Cell>destinations;   // coordinates of all the destination
void generate_destination(){

    // generate the destination coordinates by diving them with equal seperation
    destinations = vector<Cell>(num_destinations);
    int jump_space = ROWS/num_destinations;

    for(int i = 0 ; i < num_destinations ; ++i){
        Cell curr_destination;
        curr_destination.y = 0;     // all the destination are currently in first column of grid
        curr_destination.x = i*jump_space;
        destinations[i] = curr_destination;
    }
}
void take_input(){
    cin>>ROWS>>COLS;
...
// after picking all the items, which destination cell should robot visit and leave the items
int get_min_destination_distance(Cell last_cell){
    int min_distance_from_destination = 0x3f3f3f3f;
    for(auto & x : destinations){
        min_distance_from_destination = min(min_distance_from_destination,manhattan_distance(x,last_cell));
    }
    return min_distance_from_destination;
}
...
int min_all_permutation(vector<Cell>&cells,int l){
    ...
    if(l==r){
        ...
        // after picking up all the items, visit the nearest destination cell
        curr_total_time+=get_min_destination_distance(cells.back())+docking_time;
