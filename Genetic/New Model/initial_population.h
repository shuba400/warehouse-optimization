void fill_item_sequence(struct genotype& member){
  ...
  for(int i = 0 ; i < member.gene.size() ; ++i){
    ...
    member.itemSequence[i]=get_random(member.itemSequence[i]);
  }
}

genotype get_greedy_merged_random_tsp_member(){
    genotype member;
...
genotype get_greedy_merged_nn_tsp_member(){
    genotype member;
    vector<Order>mergedOrders=allOrders;
    mergedOrders=greedyMergeOrdersSize(mergedOrders);
    for(auto &m:mergedOrders) member.gene.push_back(m.subOrderIndexes);
...
genotype get_greedy_merged_optimal_tsp_member(){
    genotype member;
    member.gene.clear();
    vector<Order>mergedOrders=allOrders;
    mergedOrders=greedyMergeOrdersSize(mergedOrders);
    ...
    for(int i=0;i<member.gene.size();i++)
    {
      pair<int,vector<pair<int,Cell>>>temp=optimal_TSP(member.itemSequence[i]);
      vector<int>currItemSequence;
      for(auto &itemNumber:temp.second)
        currItemSequence.push_back(itemNumber.first);
      member.itemSequence[i]=currItemSequence;
...
genotype get_random_merged_random_tsp_member(){
  genotype member;
...
genotype get_random_merged_nn_tsp_member(){
  genotype member;
  set<int>includedOrders;
  vector<vector<int>>batches;
  vector<int>currBatch;
...
genotype get_random_merged_optimal_tsp_member(){
  genotype member;
  set<int>includedOrders;
  vector<vector<int>>batches;
  vector<int>currBatch;
